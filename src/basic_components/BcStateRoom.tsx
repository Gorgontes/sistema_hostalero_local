...
const BcStateRoom = ({ label, count, typeIcon }: Props) => {
  return (
    <div className="flex ">
      <div className="mr-auto text-primario opacity-80">{label}</div>
      <div className="font-extrabold text-base">{count}</div>
      <div className="ml-5 ">
...
const _GetFormIcon = ({ typeIcon }: _Props) => {
  switch (typeIcon) {
    case BasicStateRoom.free:
      return (
        <div className="h-6 w-6 bg-background_main rounded-full border-4 border-primario"></div>
      );
    ...
    case BasicStateRoom.occupied:
      return (
        <div className="h-6 w-6 bg-primario rounded-full shadow-primario"></div>
      );
